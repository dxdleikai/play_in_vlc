...
var append = objs => {
  ...
  list = [...list, ...objs].filter(({url}) => {
    // do not allow YouTube collection
...
window.addEventListener('message', e => {
  if (e.data && e.data.source === 'xmlhttprequest-open-open-in-vlc') {
    append([{
      url: e.data.url,
      mime: e.data.mime,
    }]);
...
document.addEventListener('canplay', ({target}) => {
  if (document.getElementById('playInVlc')) {
    document.getElementById('playInVlc').addEventListener('click', function() {
...
// document.getElementById('playInVlc').addEventListener(function() {
